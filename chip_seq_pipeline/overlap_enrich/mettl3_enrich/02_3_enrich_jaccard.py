...
result_dir = "/data5/galaxy/project/mettl3_enrich/result_parse/bedtools_jaccard"


def jaccard_test(tf_bed):
    result_file = os.path.join(result_dir, os.path.basename(tf_bed))
    os.system("bedtools jaccard -a %s -b %s > %s" % (mettl3_bed, tf_bed, result_file))


if __name__ == '__main__':
    pool = Pool()
    for tf_file in glob.glob("%s/*.bed" % tf_dir):
        pool.apply_async(jaccard_test, (tf_file, ))
    pool.close()
    pool.join()
